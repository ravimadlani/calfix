...
export const buildRecurringCsv = (series: RecurringSeriesMetrics[]): string => {
  const headers = [
    'Series Name',
    'Organizer',
    'Frequency',
  ...
  const rows = series.map(item => {
    ...
    return [
      ...
      `"${(item.organizerEmail || '').replace(/"/g, '""')}"`,
      item.frequencyLabel,
      roundTwo(item.weeklyMinutes / 60).toString(),
      roundTwo(item.monthlyMinutes / 60).toString(),
      item.peopleHoursPerMonth.toString(),
      item.internalAttendeeCount.toString(),
      item.externalAttendeeCount.toString(),
      item.acceptanceRate.toString(),
      item.cancellationRate.toString(),
      item.agendaMissing ? 'Yes' : 'No',
      `"${item.flags.join('; ')}"`,
